...
**M2 — Dependency isolation (started):** the CD Block HAL no longer weak-links or calls sat_audio_update. It exposes an optional borrowed per-block progress callback; nested reads return BUSY, and the CD streaming jukebox explicitly registers its audio service. Standalone CD clients have no implicit audio dependency. Remaining: shared math out of graphics, narrow headers, typed assets, bounded memory ownership and independent L0/L1 linking checks.
