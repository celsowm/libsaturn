...
**M3 — Rendering integration (started):** preprojected static RGB cache items can opt into the existing bounded global face painter with an explicit linear camera-depth key and pass, so they interleave in the same far-to-near order with dynamic world faces; immediate cached replay and raw VDP1 remain independent. The queue copies each quad and reports queued versus actually emitted cache items separately. Remaining: canonical typed materials/instances for cached textured surfaces, camera/view invalidation, broader mixed-scene examples and golden visual/occlusion tests, capacity/failure-injection tests, command-budget tests. Golden visual/occlusion tests, capacity/failure-injection tests, command-budget tests.
