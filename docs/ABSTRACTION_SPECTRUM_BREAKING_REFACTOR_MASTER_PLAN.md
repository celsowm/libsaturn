...
**Known limitation:** texture full updates now expose NEEDS_RECOVERY and block drawing/partial updates after a failed hardware transfer, but palette rebinding still precedes physical writes; a failure is not a hardware rollback. Rect updates still upload the full parent texture and may change caller-owned pixels before returning an error. Batch release has no scene argument and therefore cannot record its own failure in the frame; independently initiated low-level operations remain outside the optional L3 contract. Successful face dispatches do not equal physical VDP1 command counts. Render-queue unification, PAL, partial-VRAM recovery, physics complexity, CD/audio coupling and any empirically adaptive geometry dispatch algorithm remain open.
