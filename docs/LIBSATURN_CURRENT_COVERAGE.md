...
|---|---|---|---|---|
| Bare-metal startup/runtime | **SUBSTANTIAL** | Startup, linker/runtime integration, initialization and game-oriented frame lifecycle. | More system services and richer platform introspection. | `src/core`, `include/saturn/core.h`, `include/saturn/app.h` |
| Frame timing / VBlank | **SUBSTANTIAL** | Begin/end frame, VBlank wait, display-frame counter calibrated from SH-2 timing. | Broader interrupt/timer abstraction. | `include/saturn/video.h`, `src/hal/scu.*` |
...
| Mesh/model pipeline | **SUBSTANTIAL** | Mesh/model runtime plus offline OBJ/MTL/image baking for VDP1-compatible faces. | Runtime asset streaming, broader import formats and richer material model. | `include/saturn/mesh3d.h`, `include/saturn/model3d.h`, `tools/import_model.py` |
...
| SCSP PCM playback | **PARTIAL** | PCM S8/S16 sounds, resident sound data, looping, voices, volume, pan, pitch and voice statistics/stealing. | Streaming audio, richer envelopes/modulation, effects and synthesis. | `include/saturn/audio.h`, `src/hal/scsp.*` |
...
| Runtime filesystem-independent asset API | **PARTIAL** | Bounded logical asset handles, metadata, raw-data access, typed texture/font/sound loaders, and generated C registration for embedded/physical manifest entries are independent of the current storage representation. | Concrete CD/RAM-cart backends and non-resident asset refill. | `include/saturn/asset.h`, `src/core/asset_api.cpp`, `tools/generate_asset_manifest.py`, `tools/generate_asset_registry.py` |
