...
| Runtime filesystem-independent asset API | **PARTIAL** | Bounded logical asset handles, metadata, caller-owned partial reads, typed texture/font/sound loaders, and generated C registration for embedded/physical manifest entries are independent of the current storage representation. | Concrete CD/RAM-cart backends and non-resident typed asset refill. | `include/saturn/asset.h`, `src/core/asset_api.cpp`, `tools/generate_asset_manifest.py`, `tools/generate_asset_registry.py` |
