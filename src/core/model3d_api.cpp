...
extern "C" sat_result_t sat_model_copy_to_mesh(const sat_model_asset_t* asset, sat_mesh_t* mesh) {
    ...
    if (mesh->vertex_cap < asset->vertex_count || mesh->face_cap < asset->face_count) {
        return SAT_ERR_CAPACITY;
    }
    for (uint16_t i = 0; i < asset->vertex_count; ++i) {
        mesh->vertices[i] = asset->vertices[i];
    }
    for (uint32_t i = 0; i < (uint32_t)asset->face_count * 4u; ++i) {
        mesh->indices[i] = asset->indices[i];
    }
...
extern "C" sat_result_t sat_model_upload_textures(
    ...
    if (asset == nullptr || out_textures == nullptr) {
        return SAT_ERR_INVALID_ARG;
    }
    if (saturn::core::model3d::validate(asset) != SAT_OK) {
        return SAT_ERR_INVALID_ARG;
    }
    if (out_cap < asset->texture_count) {
        return SAT_ERR_CAPACITY;
    }
    /* One palette upload per palette slot, then one pixel upload per texture.
     * Palettes live as consecutive 256-entry blocks from palettes_rgb555. */
    ...
    for (uint16_t t = 0; t < asset->texture_count; ++t) {
        ...
        const sat_result_t st = sat_tex_upload_indexed8_pixels(
            &out_textures[t], src->pixels, src->width, src->height, bank);
        if (st != SAT_OK) {
            return st;
        }
    }
...
extern "C" sat_result_t sat_model_bind_draw(
    ...
    if (texture_count < asset->texture_count) {
        return SAT_ERR_INVALID_ARG;
    }
...
extern "C" sat_result_t sat_model_compute_bounds(
    const sat_model_asset_t* asset,
    sat_vec3_t* out_min,
    sat_vec3_t* out_max
) {
...
extern "C" sat_result_t sat_model_compute_center(
    const sat_model_asset_t* asset,
