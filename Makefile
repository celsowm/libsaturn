...
$(ELF): $(CRT_OBJS) $(ALL_APP_OBJS) $(LIBRARY)
	...
	$(PYTHON) $(TOOLS)/check_no_static_ctors.py --elf $@ --objdump $(TARGET)-objdump
...
# -- Header dependencies ----------------------------------------------
# Generated by DEPFLAGS above. The leading '-' keeps the first build, when no
# .d files exist yet, from failing.
ALL_DEP_FILES := $(shell find $(BUILD_DIR) -name '*.d' 2>/dev/null)
