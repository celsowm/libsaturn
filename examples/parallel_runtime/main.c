...
#define GEOMETRY_OBJECT_COUNT 12u
...
static void make_geometry_scene(void) {
    g_geometry_vertices[0] = (sat_vec3_t){-SAT_FX16_ONE, -SAT_FX16_ONE, 0};
    g_geometry_vertices[1] = (sat_vec3_t){ SAT_FX16_ONE, -SAT_FX16_ONE, 0};
    g_geometry_vertices[2] = (sat_vec3_t){ SAT_FX16_ONE,  SAT_FX16_ONE, 0};
    g_geometry_vertices[3] = (sat_vec3_t){-SAT_FX16_ONE,  SAT_FX16_ONE, 0};
...
static void update_geometry_world(uint32_t frame) {
    for (uint16_t i = 0u; i < GEOMETRY_OBJECT_COUNT; ++i) {
        ...
        const int16_t z = (int16_t)(8 + (i % 3u) * 3);
...
static void validate_geometry_output(void) {
    ...
    for (uint16_t i = 0u; i < g_geometry_batch.metrics.prepared_faces; ++i) {
        if (g_geometry_keys[i] != g_geometry_reference_keys[i] ||
            g_geometry_faces[i].projected.x[0] !=
                g_geometry_reference_faces[i].projected.x[0] ||
            g_geometry_faces[i].projected.y[0] !=
                g_geometry_reference_faces[i].projected.y[0]) {
            g_geometry_validation = 0u;
            return;
        }
    }
...
static void draw_status(uint32_t wait_ticks, uint32_t submission_ticks) {
    ...
    sat_ascii_font_draw_text_screen_indexed8(&g_font,
        g_validation != 0u && g_geometry_validation != 0u
...
static void cycle_backend(void) {
    ...
    if (g_geometry_handle_valid != 0u) {
        if (sat_parallel_wait(g_geometry_handle, STARTUP_TIMEOUT) != SAT_OK) ++g_errors;
        (void)sat_parallel_release(g_geometry_handle);
...
int main(void) {
    ...
    for (;;) {
        ...
        if (g_geometry_handle_valid != 0u) {
            if (sat_parallel_wait(g_geometry_handle, STARTUP_TIMEOUT) != SAT_OK) {
                ++g_errors;
            } else {
                if (g_frame == 0u) validate_geometry_output();
