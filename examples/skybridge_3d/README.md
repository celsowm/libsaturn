...
> Historical note: older paragraphs below describe the removed queue/immediate
> facade. Current source uses the canonical caller-owned `sat_scene_t` path;
> see `docs/RUNTIME_OWNERSHIP_AND_EXAMPLE_LEDGER.md`.
...
Move with the digital D-pad (camera-relative), press **A** to jump (release early for a shorter jump), press **B / C** to rotate the follow camera by 15 degrees, press **Y** to toggle the extra platform/yaw/surface diagnostic HUD, hold **Z** for an active brake, and press **START** to pause/resume. The second HUD row always shows the player's signed **X, Y, Z world coordinates** to one decimal place (not the interpolated camera position). Reach the gold arch on the tenth platform to complete the course. **START on a course's completion screen advances through Courses 1 → 2 → 3 → 4, then restarts Course 1.** To select any course without finishing the previous one, pause with **START**, press **X** to cycle through the four courses while remaining paused, then press **START** to play; choosing a course resets its time, gems and checkpoints. **All eight gems are optional bonuses**; the finish never requires a minimum gem count, and the end screen shows how many you collected. The fourth and seventh platforms are checkpoints in all four courses. Falling into the sea respawns you at the last checkpoint in the current course, retaining its collectibles. Course 1 retains the sideways moving platform and timed collapsing bridge. **Course 2** has ten different platforms with alternating narrow/wide landing zones, four elevators moving vertically at staggered phases, and an elevated finish. **Course 3** has ten wide, long piers, with six genuinely open rectangular holes cut out of their floors. Some openings leave a side path, while others nearly span the entire walkway and encourage jumping. A yellow-orange rim marks each opening. **Course 4** has six genuine hinged seesaw ramps, with fixed checkpoint and finishing piers: the board dips under the pig's weight and gradually levels out after the pig jumps away. The moving decks carry the pig up and down while grounded; gems and the contact shadow follow the same physical platform height.

This demo has no mandatory RAM cartridge, external art downloads, controller extension, CD streaming or game save. The pig source is checked in at `assets/pig_source.glb` and is converted at build time by the same generic animated-GLB importer used by the other model examples; the generated Saturn mesh is embedded in the program. The generated palette, ocean, sky and PCM all come from C source. A startup loading panel reports completed initialization work and tracks real ocean rows generated, not a fixed timer.
...
The source is split by concern; `skybridge.h` is the shared contract (settings,
world palette, painter passes, shared state, entry points):

| File | Owns |
| --- | --- |
| `main.c` | start-up order, course start, resource plan, frame loop |
| `world.c` | the scene painter pass: decks, seesaws, lifts, insets, contact shadow, fade slots |
| `pig.c` | imported pig: animation decode on Master or Slave, instance submission |
| `gems.c` | collectible gems: shared octahedron, batch preparation split across both SH-2s |
| `materials.c` | world palette, shared solid-colour pool, deck inset textures |
| `background.c` | VDP2 sky and RBG0 sea, VDP1 cloud band |
| `audio.c` | synthesised effects, music loop, game-event cues |
| `hud.c` | boot panel, HUD, debug overlay |
| `metrics.c` / `telemetry.c` | per-frame metrics; validation telemetry block for the harness |
| `game.h` / `scenery.h` | pure game rules and procedural scenery (host-tested) |
...
## Generic scene-wide painter

This game uses the canonical `sat_scene_t` contract: caller-owned face and sort storage is initialized once, then each frame submits visible platform, pig and gem faces/instances. LibSaturn computes camera depth, clips, sorts all accepted faces deterministically and lowers them to VDP1 before the protected HUD pass. The platform and game rules stay here, but the former game-specific object queue, actor-depth helpers and immediate model facade are gone. See `docs/SCENE3D_PAINTER_QUEUE_PLAN.md` for the historical rationale and current limitations.

The three explicit passes are **0 world decks, 1 supporting deck, 2 pig and gems**,
selected through `g_active_pass` / the instance `pass` field. They are not
cosmetic: a deck top is one large quad whose average camera depth competes with
an actor standing on it, so with everything in a single pass the deck wins as
the player walks along it and paints over the pig from the feet up until only a
sliver is left. Measured on a straight walk down course 1, the pig fell from 274
visible pixels to 9; with the passes separated it holds at ~380. Passes 0/1 preserve the sample's original large-platform painter workaround, not a hardware Z-buffer. Only objects in the same pass are ordered against one another; when two large meshes intersect, a single anchor cannot guarantee perfect per-pixel visibility. The VDP2 distance-fade priority, platform activation, gem physics and HUD remain managed by their existing systems.
...
START after Course 1 completion begins Course 2; START after Course 2 begins Course 3; START after Course 3 begins Course 4; START after Course 4 restarts Course 1. Each transition resets that course's gems, time and checkpoints. While paused, X cycles through all four courses and leaves the picker paused; START begins the selected course, allowing direct testing without completing preceding courses. The title HUD shows C1/C2/C3/C4 and the X/Y/Z row remains live. Checkpoints remain on the fourth and seventh decks; the goal remains accessible without collecting any gems. The original course's special moving/collapse objects are activated by each platform's kind, not a globally hard-coded platform index. The new host regression runs all four elevators through a complete 180-tick period, checks grounded carry and jump detachment, moving-height gem collision, both course resets, a narrow-deck footprint and completion with zero gems.
...
The player now comes from the checked-in `assets/pig_source.glb`. The build sends that native GLB through `tools/import_model.py --target saturn`, baking its authored animation clips, indexed face colors and Saturn-compatible 16.16 mesh data into `build/generated/skybridge_3d/pig_model.c/.h`. Runtime code only uploads and poses the generated asset; it does not parse GLB or use zlib on the Saturn. The pig remains limited to the previous 4×4×5 collision volume and stays opaque even when distant platforms fade through VDP2 color calculation. Neither pickup radius nor physics, checkpoint, course or elevator handling changes.
...
## Course 4: physical hinged ramps and seesaws

Course 4 adds six **real sloped decks**, indices 1, 2, 4, 5, 7 and 8,
alternating with fixed starting, checkpoint and finishing piers. It reuses
the existing 10-platform scene queue and original jump impulse. Each
board has a centre hinge, a distinct colored surface and two differently
elevated ends. The **same** `sb_platform_surface_y()` function calculates
the deck vertices used by `draw_seesaw()`, the pig's grounded foot
position, landing detection, shadow placement and gem's hinge height.
It is NOT a flat platform with decorative tilt applied only to its mesh.

The height difference at each end is bounded to three world units
(relative to the hinge). The pig's position along the board determines
its target tilt: a pig near the forward end depresses that end; when it
walks to the rear, the tilt gradually reverses. An occupied plank moves
towards the weight-dependent target by at most 1/8 unit per simulation
tick; unoccupied planks slowly settle towards horizontal (1/20 unit/tick).
No physics middleware or floating-point trigonometry is required.
An attached pig remains on the exact sloped surface uphill and
downhill, and **jumping immediately detaches it**. An airborne pig can
land only by crossing the ramp surface from above, not by teleporting
upward through it. Fixed checkpoint piers at indices 3 and 6 retain
progress and gems remain optional.

**To select Course 4 directly:** START to pause, X three times to
cycle from C1 to C4, then START to play. From Course 3's completion screen, START
advances into Course 4; finishing Course 4 wraps to Course 1. The HUD
shows `C4`, and the start hint reads `RIDE THE TILTING RAMPS`.
The optional Y diagnostic HUD displays signed `T` (the current seesaw's
+Z-end rise relative to its hinge) while the pig is standing on a ramp.
The new host tests run all six seesaws through near-end and far-end
loading, verify signed physical tilt and pig/surface height agreement,
jump detachment, falling below ramps, checkpoint and four-course
progression. These tests do not substitute for an actual emulator
video showing the two ends move oppositely and the HUD update when
the pig jumps between adjacent seesaws. This course reuses the known
VDP1 screen clipping path; watch for the previously reported HUD
disappearance under heavy polygon raster load.

## Course 3: long platforms with real holes

The third course uses ten fixed, elongated decks, with six different
rectangular apertures (on decks 2, 3, 5, 6, 8 and 9). The wide openings
alternate between left/right offsets and two nearly full-width cuts that
encourage the pig to jump. The leading/trailing solid areas allow the same
original short/long jump timings to cross the gaps without adding a new
jump power-up. The fixed second platform **in Course 1** is unchanged;
none of the existing first/second course platform data is modified.

**Not decals:** `sb_course3_holes` is an immutable array shared by gameplay
and drawing. `sb_deck_slices()` returns the solid rectangles around each
hole; `stage_box()` draws only those slices, and
`sb_supported_footprint()` checks whether nearly the entire player
footprint lies on one of those same solid rectangles. The aperture has no
top polygon, no underside and no hidden support; if the pig steps inside,
gravity makes it fall. Jumping over it can land on the intact part past the
opening. For an airborne pig, the upward head collision also ignores the
empty opening. Optional gems stay on solid deck centres, outside the holes;
checkpoints remain on decks 4 and 7, and the finish needs no gems.

The yellow-orange strips are on *solid ground immediately around* the hole,
not over it. Course 3 is a bounded geometric extension of the existing
10-platform scene queue, rather than ten giant single-surface sprites or
new VDP2 bitmap uploads. A long deck with an opening issues more VDP1
commands than an uninterrupted deck; observe frame rate and HUD visibility
in the emulator, especially when jumping across consecutive holes.
`tests/host/test_skybridge_game.cpp` checks the six holes' solid areas,
non-overlapping deck slices, falls from an opening, midair passage,
checkpoint resets and a gem-free win. Host tests cannot establish that
the Saturn hardware rasterizer remains within frame budget.
...
To verify the fade and the repaired controls, walk right at default camera yaw (screen-right should move toward world -X), stand at both the near and far edges of the first, second, and checkpoint platforms (the pink pig and each platform's solid sidewalls must remain visible), then compare emulator screenshots with the camera approaching the second or third platform; the platform should progress through several transparency steps as its view depth crosses 134 down to 66, rather than suddenly appearing at a culling boundary. The near platform and player must stay opaque while the ocean and sky remain visible through the far one. Also check that an uncollected gem on a fading deck fades with it rather than staying a solid yellow diamond over a half-transparent platform. The `distance_fade_3d` example independently verifies the supported LibSaturn indexed-sprite/VDP2 blend path and shares the same `sat_fade3d_slot` binding. `make test` checks fade quantization, the slot mapping and the anti-oscillation band in host tests; visual correctness on the chosen emulator/hardware still requires actual screenshots.

Press **Y** for the debug rows. The second one is a budget row --
`F <faces>/<cap>  C <commands>/<cap>  P <pig faces>/<total>  FULL` -- reading
the painter queue and the VDP1 command list (`sat_vdp1_command_stats`) right
after the world flush. Running out of either drops geometry silently, because
every world submission treats `SAT_ERR_CAPACITY` as an optional decoration, so
a half-drawn character otherwise looks like a clipping or animation bug. If the
pig looks wrong while `F`, `C` and `P` are all well under their caps, the cause
is paint ORDER, not budget.
