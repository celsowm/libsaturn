...
- VDP1 draws nearby platforms, the player cube, pickups, checkpoints and finish arch as RGB polygons with Gouraud-shaded tops and a shared INDEX8 16×16 platform-inset texture (palette bank 3). **Platforms at view depth 66–134 gradually fade**: `sat_fade3d_eval` selects one of eight VDP2 color-calculation ratios, and the entire distant platform, inset, collectible and checkpoint markers are submitted as indexed VDP1 distorted sprites using a pre-uploaded set of solid-color 16×16 textures (palette bank 4). The fade priority is 6, above the RBG0 sea at 5; the opaque RGB geometry, player and HUD retain priority 7. This is real background blending, **not** VDP1 mesh stippling, Gouraud darkening or a hard cutoff. Basic per-object depth sorting is used because VDP1 has **no depth buffer**. The near-player contact shadow remains a checkerboard-mesh polygon, not true alpha. Font glyphs use palette bank 2 and are drawn after the world.
- The frame uses a transparent VDP1 erase and calls `sat_vdp2_layers_commit`, **then** `sat_vdp2_sprite_color_calc_commit`, before `sat_begin_frame`. The ordering is required: the generic VDP2 layer commit otherwise overwrites the faded-sprite priority selector. Do not replace the frame lifecycle with `sat_app_frame_begin`, which clears VDP1 to opaque and can hide the VDP2 scenery.
...
This is the **first playable implementation**, not the full production acceptance of `docs/SKYBRIDGE_3D_PLATFORMER_EXAMPLE_PLAN.md`. It deliberately uses simple low-poly geometry, no GLB assets, no bespoke runtime alpha engine, no optional Slave SH-2, DMA, cart cache, save files, or advanced per-line water effects. The current broad-phase is a ten-AABB scan, not the large-world spatial grid. The follow camera is fixed-distance and can intersect arbitrary additional geometry if you modify the level; the world is laid out with generous sight lines. The simple center-based painter ordering may show incorrect overlap where meshes intersect. Indexed color-calculated platforms blend against the **VDP2** backdrop, not a separately composited earlier VDP1 object; distant overlapping platform geometry is therefore intentionally kept sparse. The transition from near RGB polygon materials to distant indexed materials is not identical per-pixel and may be visible on close inspection; alpha is not supported for arbitrary Gouraud RGB polygons by the existing API. Fixed 60 Hz game ticks are capped at three catch-up ticks per displayed iteration.
